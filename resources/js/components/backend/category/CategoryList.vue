<template>
  <div class="container">
    <div class="w-full">
      <title-section
        :title="'Informasi kategori berita'"
        :subtitle="'Semua informasi terkait  berita'"
      ></title-section>
        ...
        <div class="w-1/3 border border-grey-light p-8 bg-white rounded">
          ...
          <table class="table-auto block">
            ...
            <tbody>
              ...
              <tr
                vue-if="!isLoading && categorys.length > 0"
      ...
      <modal name="add-category" class="rounded" height="auto" :scrollable="true">
        <div class="p-6">
          <div class="text-xl block w-full font-semibold mb-1 pb-4 border-b border-grey-light">
            <i class="fa fa-info-circle text-grey-dark"/> Tambah data kategori
          </div>
          <div class="flex">
            <div class="w-full">
          ...
          <div class="w-full mt-8 block">
            ...
            <a v-if="isLoadingButton" href="#" class="btn-blue btn-md">
              <spinner></spinner>
            </a>
          </div>
        </div>
      ...
      <modal name="edit-category" class="rounded" height="auto" :scrollable="true">
        <div class="p-6">
          <div class="text-xl block w-full font-semibold mb-1 pb-4 border-b border-grey-light">
            <i class="fa fa-info-circle text-grey-dark"/> Ubah data kategori
          </div>
          <div class="flex">
            <div class="w-full">
              <small class="label mt-6">Nama kategori</small>
...
export default {
  ...
  methods: {
    ...
    submitData() {
      ...
      axios
        .post("/api/submit-category/", { data: this.new_data })
        .then(res => {
    ...
    updateData() {
      ...
      axios
        .post("/api/update-category/", { data: this.edit_data })
        .then(res => {
          if (res.data.error == "validation") {
            this.isLoadingButton = false;
          } else {
    ...
    deleteData(id) {
      ...
      this.$modal.show("dialog", {
        ...
        text:
          "Apakah anda ingin menghapus kategori ini?, Apabila menghapus kategori ini maka semua berita yang terkait dengan kategori ini juga akan dihapus",
        buttons: [
          {
            title: "✔ Lanjutkan",
            default: true,
